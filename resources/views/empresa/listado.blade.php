                ...
                @foreach($listado_empresas as $empresa)
                    <tr>
                        ...
                        <x-form.td>
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <img class="h-10 w-10 rounded-full" src="{{asset("storage/logos/{$empresa['empresa']->logo}")}}">
                                </div>
                                <div class="ml-4">
                        ...
                        <x-form.td class=" text-right text-sm font-medium">
                            <a href="{{route("empresas.edit",[$empresa['empresa']])}}" class="text-indigo-600 hover:text-indigo-900"><img class="w-8 h-8" src="{{asset("images/editar.png")}}"></a>
                        </x-form.td>
                        <x-form.td>
                            <form action="{{route('empresas.destroy', [$empresa['empresa']])}}" method="post">
                                @method("delete")
                                @csrf
                                <input type="image" src="{{asset("images/borrar2.jpg")}}" class=" w-8 h-8 text-indigo-600 hover:text-indigo-900" />
                            </form>
                        </x-form.td>
                    </tr>
                @endforeach
